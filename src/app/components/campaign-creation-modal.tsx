...
import {
  ...
  Lightbulb, Award, Hash, AlertCircle,
} from "lucide-react";
...
const CONTENT_TYPES: ContentTypeDef[] = [
  { id: "blog-post", label: "Blog Post / Recap", Icon: FileText, color: "#60A5FA", credits: 20 },
...
const LIBRARY_ASSETS = [
  ...
  { id: 4, name: "Brand Guidelines 2024.pdf", type: "pdf", size: "3.2 MB" },
];
...
export function CampaignCreationModal({ isOpen, onClose, onComplete }: CampaignCreationModalProps) {
  ...
  const generateScheduledItems = () => {
    ...
    // Create items based on content type counts
  ...
  const totalCredits = useMemo(() => {
    return CONTENT_TYPES.reduce((sum, type) => {
      return sum + type.credits * contentTypeCounts[type.id];
    }, 0);
  }, [contentTypeCounts]);
  ...
  const contentTypeDistribution = useMemo(() => {
    return CONTENT_TYPES.map((type) => ({
      ...type,
      count: contentTypeCounts[type.id],
      percentage: totalItems > 0 ? (contentTypeCounts[type.id] / totalItems) * 100 : 0,
    }));
  }, [contentTypeCounts, totalItems]);
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {step === 2 && (
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className="text-sm font-bold text-foreground block mb-3">Include from project library</label>
          ...
          {step === 3 && (
            <div className="p-6 space-y-5">
              <div>
                <h3 className="text-base font-bold text-foreground mb-1">Choose your content mix</h3>
                <p className="text-sm text-muted-foreground">
                  Set how many of each content type you want to generate for this campaign.
                </p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div
                        ...
                        style={{ backgroundColor: `${type.color}18`, border: `1px solid ${type.color}30` }}
                      >
                        <Icon className="w-5 h-5" style={{ color: type.color }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-foreground">{type.label}</p>
                        <p className="text-xs text-muted-foreground">{type.credits} credits each</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() =>
                          ...
                          className="w-8 h-8 rounded-lg border border-border bg-secondary hover:bg-card text-muted-foreground hover:text-foreground transition-colors flex items-center justify-center"
                        >
                        ...
                        <input
                          ...
                          min="0"
                          max="50"
                          className="w-16 px-3 py-2 bg-secondary border border-border rounded-lg text-sm text-center font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
                        />
                        <button
                          onClick={() =>
                            setContentTypeCounts((prev) => ({
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-primary/5 border border-primary/20">
                ...
                <span className="text-2xl font-black text-primary tabular-nums">{totalItems}</span>
              </div>

              {/* Distribution bar */}
              <div>
                <label className="text-sm font-bold text-foreground block mb-3">Content Type Distribution</label>
              ...
              {/* Total credits */}
              <div className="flex items-center justify-between p-4 rounded-xl bg-emerald-500/[0.06] border border-emerald-500/[0.15]">
                <span className="text-sm font-bold text-foreground">Estimated Credits</span>
                <span className="text-xl font-black text-emerald-400 tabular-nums">{totalCredits}</span>
              </div>
            </div>
          )}
